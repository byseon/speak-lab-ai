  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-center gap-2 text-sm text-[var(--session-foreground)]">
          <span
        ...
        <div className="relative mt-6 rounded-xl border border-white/15 bg-black/20 p-4 backdrop-blur-sm sm:p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--session-foreground-muted)]">
            {t("hero.session.examinerLabel")}
          </p>
          <p className="mt-2 text-sm leading-relaxed text-[var(--session-foreground)] sm:text-base">
        ...
        <div className="relative mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {SCORES.map(([key, value]) => (
            <div
              key={key}
              className="rounded-lg border border-white/10 bg-black/15 px-2 py-2 text-center"
            >
              <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--session-foreground-muted)]">
                {t(`hero.session.criteria.${key}`)}
              </p>
              <p className="mt-0.5 text-sm font-bold text-[var(--session-foreground)]">
                {value}
              </p>
            </div>
          ))}
