...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "SpeakLab — Practice IELTS Speaking with a real AI examiner",
      },
      {
...
function CTA({ className = "" }: { className?: string }) {
  ...
  return (
    <Button
      asChild
      size="lg"
      className={`w-full sm:w-auto shadow-[var(--shadow-soft)] ${className}`}
    >
      <Link to="/signup">
    ...
    <div className="min-h-screen bg-background font-sans text-foreground antialiased">
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-card/60 text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-3">
                      {t("mockVsPractice.columns.mock")}
                    </th>
                    <th scope="col" className="px-4 py-3">
                      {t("mockVsPractice.columns.practice")}
                    </th>
                  </tr>
                </thead>
                <tbody>
      ...
      <footer className="border-t border-border bg-primary text-primary-foreground">
        <Section className="text-center">
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">{t("footer.headline")}</h2>
